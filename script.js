...
function replaceName() {
    ...
    if (url.searchParams.has('firstname') && url.searchParams.has('lastname')) {
        var name = url.searchParams.get('firstname') + " " + url.searchParams.get('lastname');
...
function initReveal() {
    // Fix separator regexes to tolerate both Unix (LF) and Windows (CRLF) line breaks
    document.querySelectorAll('[data-markdown]').forEach(function(el) {
        var sep = el.getAttribute('data-separator');
        if (sep && !sep.includes('\\r?')) {
            el.setAttribute('data-separator', sep.replace(/\\n/g, '\\r?\\n'));
        }
        var vsep = el.getAttribute('data-separator-vertical');
        if (vsep && !vsep.includes('\\r?')) {
            el.setAttribute('data-separator-vertical', vsep.replace(/\\n/g, '\\r?\\n'));
        }
    });

    // load all csses
    loadCsses([
        'https://cdnjs.cloudflare.com/ajax/libs/reveal.js/4.4.0/reset.min.css',
        'https://cdnjs.cloudflare.com/ajax/libs/reveal.js/4.4.0/reveal.min.css',
        'https://cdnjs.cloudflare.com/ajax/libs/reveal.js/4.4.0/theme/black.min.css',
        'mycss.css',
        'https://cdnjs.cloudflare.com/ajax/libs/reveal.js/4.4.0/plugin/highlight/monokai.min.css'
    ], function() {

        // load all scripts
        loadScripts([
            'https://cdnjs.cloudflare.com/ajax/libs/reveal.js/4.4.0/reveal.min.js',
            'https://cdnjs.cloudflare.com/ajax/libs/reveal.js/4.4.0/plugin/notes/notes.min.js',
            'https://cdnjs.cloudflare.com/ajax/libs/reveal.js/4.4.0/plugin/markdown/markdown.min.js',
            'https://cdnjs.cloudflare.com/ajax/libs/reveal.js/4.4.0/plugin/highlight/highlight.min.js',
            'https://cdnjs.cloudflare.com/ajax/libs/reveal.js/4.4.0/plugin/math/math.min.js'
        ], 'anonymous', 'no-referrer', function() {

            console.log('Initializing Reveal.js');
            var revealpromise = Reveal.initialize({
                hash: true,
                slideNumber: true,
            ...
            revealpromise.then(function() {
                ...
                console.error('Error during Reveal.js initialization:', err);
            });
